...
│  Page        │  │  (A4/Letter，固定物理尺寸)    │  │  Page    │  │              │
└──────┬───────┘  └──┬──────────┬──────────┬────┘  └────┬─────┘  └──────┬───────┘
       │             │          │          │             │               │
  n Shapes      Header       Footer    Document        n Shapes     1 Sheet
               (page 层)   (page 层)  （body，              (grid)
                                       page 层）
                                          │
                                   n nested Paragraph
                                   ├─ canvas 文字块
...
| 层级 | 所属 | 类型 | 说明 |
|------|------|------|------|
| Graph | — | `DocumentGraph` extends `graph` | 文档级配置（页面尺寸、边距、样式表） |
| Page | Graph | `DocumentPage` extends `page` | n 个 A4/Letter 页，自动分页 |
| Header | **Page** | `PageHeader` extends `shape` | 页眉，属于 Page，不属于 Document body |
| Footer | **Page** | `PageFooter` extends `shape` | 页脚，属于 Page，不属于 Document body |
| Document | **Page** | `Document` extends `container` | 正文 body，属于 Page，承载所有段落内容 |
| Paragraph | Document | `Paragraph` extends `shape` | 段落，canvas 文字渲染，支持嵌套 |
| Inline Shape | Paragraph | 任意 shape | 嵌入 Paragraph 的图形，支持两种布局：<br>· **上下排列**（inline-block，占段落行内空间）<br>· **文字环绕**（浮动/绝对定位，文字流绕排） |

**关键约束**
- `Header` / `Footer` / `Document` 三者平级，均挂在 `DocumentPage` 下
- `Document` 不包含 Header/Footer，仅管理 body 段落流
- 段落内嵌入的 Shape 通过布局属性（`inlineLayout: 'block' | 'wrap'`）决定文字排版行为
