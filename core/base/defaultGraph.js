...
import AnnaEditor from '../editor/default/anna-editor.js';
import {EVENT_TYPE} from '../../common/const.js';
import {annaCKEditor} from '../editor/default/annaCKEditor.js';
import {editorCommand} from '../history/commands.js';
...
const defaultGraph = (div, title) => {
  ...
  self.initialize = async () => {
    if (self.enableText) {
      ...
      self.editor = await AnnaEditor.create({}, {generateId: self.uuid});
  ...
  const getShapeIdByRootName = (rootName) => {
    // anna-page:2rt02i_5zx9or_configuration，分隔之后 ["anna-page:2rt02i", "5zx9or", "configuration]
  ...
  self.createEditor = (shape) => {
    ...
    // 不同的页面中shape的id可能一样，因此，这里加上pageId保证唯一性.例如: anna-page:2rt02i_5zx9or_configuration
    const editableName = `${shape.page.id}_${shape.id}_${shape.page.mode}`;
    const editor = annaCKEditor(shape, self.editor, editableName);
