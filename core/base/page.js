...
import {shapeComment, shapeComments} from '../shapes/rectangle.js';
import {popupMenu} from '../contextToolbar/popupMenu.js';
import {pluginMeta} from '../configuration/pluginMeta.js';
import {addCommand, deleteCommand, positionCommand} from '../history/commands.js';
import {guideLineUtil} from '../utils/guideLineUtil.js';
import {copyPasteHelper} from '../actions/copyPasteHelper.js';
import {shapeManager} from './shapeManager.js';

/**
 * wantedShape — 记录用户选中的"待创建形状"状态。
 *
 * 当用户在工具栏点选某个形状类型后，通过 page.want(type, properties) 设置，
 * 下次点击画布空白处时，page 会从此对象读取类型和属性来创建形状。
 * 创建完成后自动调用 clear() 重置（单次使用）。
 */
...
/**
 * Page — 一页画布，Graph 中的最顶层容器。
 *
 * Page 继承自 Container，同时也是所有 Shape 的根容器（page.isTypeof('container') === true）。
 *
 * 职责：
 *   1. 管理本页所有 Shape（通过 shapeManager）
 *   2. 响应用户交互（通过 interactDrawer + mouseActions + keyActions）
 *   3. 控制页面缩放、平移、滚动（scaleX/scaleY/x/y）
 *   4. 协调三个 Drawer 分层渲染：pageDrawer（背景）、interactDrawer（交互）、animationDrawer（动画）
 *   5. 管理演示动画队列（animations[]、animationIndex）
 *   6. 支持多种工作模式（configuration/display/presentation/view/runtime/history）
 *   7. 持有 isReady 标志，控制初始化完成前不广播 Shape 变化
 *
 * @param {HTMLElement} div       宿主 DOM 容器
 * @param {Graph}       graph     所属 Graph
 * @param {string}      name      页面名称
 * @param {string}      id        页面 id（不传则自动生成 anna-page:xxx）
 * @param iDrawer   交互层绘制器工厂（默认 interactDrawer，可注入测试用 mock）
 * @param pDrawer   页面背景绘制器工厂（默认 pageDrawer）
...
const page = (div, graph, name, id, iDrawer = interactDrawer, pDrawer = pageDrawer, aDrawer = animationDrawer) => {
  ...
  div.innerHTML = '';
  // Page 以 container 为基础创建，初始尺寸等于宿主 div 的尺寸
  const self = container(id, 0, 0, div.clientWidth, div.clientHeight, graph);
  // 清除从 Container 继承来的所有 detection，Page 自己管理监听
  ...
  // Page id 格式：anna-page:{uuid}，方便在 DOM 和日志中快速识别
  (id === undefined) && (self.id = `anna-page:${graph.uuid()}`);
  self.uuid = graph.uuid();
  // sm（shapeManager）：管理本页所有 Shape 的扁平化列表和查询
  self.sm = shapeManager(self);

  // shapesAs：类型替换映射，如 {rectangle: 'myCustomRect'}，
  // 使本页内的 rectangle 以 myCustomRect 类型渲染（不影响序列化）
  self.shapesAs = {};
  self.graph = graph;
  // page 指向自身（Shape 内部通过 self.page 访问所在 Page）
  self.page = self;
  ...
  /**
   * areas：将画布按 areaStep（100px）网格分块，Shape 按坐标索引到对应格子。
   * 鼠标点击时先定位到对应格子，再在小范围内精确查找，避免遍历所有 Shape（性能优化）。
   */
  self.areas = {};
  self.areaStep = 100;
  self.disableContextMenu = false;
  // operationMode：当前操作模式，SELECTION（框选/点选）或 DRAG（拖拽画布平移）
  self.operationMode = PAGE_OPERATION_MODE.SELECTION;
  // selectionStrategy：选中时是否将形状置顶（BRING_TO_FRONT）或只高亮边框（HIGHLIGHT_BOUNDARIES）
  ...
  /**
   * isReady：页面是否已完成初始化并可以开始广播 Shape 变化。
   * 为 false 时，Shape 属性的变化不会触发协同广播，避免初始化阶段产生大量无效消息。
   * 由 page.reset() 在初始化完成后设置为 true。
   */
  self.isReady = false;
  /**
   * page中形状会响应键盘事件
  ...
  self.moveTo = (newX, newY) => {
    let newXVal = newX;
    let newYVal = newY;
    // 只在页面有图形时才进行边界约束；空页面时 getShapeFrame() 返回 MAX/MIN_VALUE 哨兵值，
    // 不能参与计算，否则会将 page.x/y 钳位到 ±Number.MAX_VALUE
    if (self.sm.getShapes().length > 0) {
...
const setMouseActions = (pageVal) => {
  ...
  pageVal.mouseRightClick = position => {
    ...
    if (pageVal.showContextMenu()) {
      popupMenu(position.x, position.y, pageVal.switchMouseInShape(position.x, position.y, s => true), null, position.e);
    }
    return false;
  };
  ...
  pageVal.onMouseDown = position => {
    if (!pageVal.wantedShape.isEmpty()) {
      pageVal.getFocusedShapes().forEach(s => s.unSelect());
      // 同步保存类型和属性，立即清除 wantedShape，保证 mouseup 时 isEmpty()===true，
      // 从而让 whiteboard.js 的 mouseup 监听器能正确把工具切回 select。
      const pendingType = pageVal.wantedShape.getType();
  ...
  pageVal.addDetection(['wantedShape', 'ongoingShape', 'inHandDrawing', 'eraser'], (property, value, preValue) => {
    ...
    pageVal.setCursor();
    // cursor detection 被 inReacting 防重入阻断，需在此处直接刷新交互层，确保光标样式立即生效
    pageVal.invalidateInteraction();
  });
