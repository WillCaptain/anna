...
import {shapesIndexChangedCommand} from '../history/commands.js';
...
const writer = (sm, page, enableCollaboration, enableCommands) => {
  ...
  self.moveTop = (shapes) => {
    ...
    const minIndex = shapes.min(s => s.index);
    shapes.filter(s => s.serializable).orderBy(s => s.index).forEach(s => {
  ...
  self.moveBottom = (shapes) => {
    ...
    const maxIndex = shapes.max(s => s.index);
  ...
  self.swapShape = (shapeA, shapeB) => {
    ...
    if (!isIndexValid(aIndex) || !isIndexValid(bIndex)) {
      return;
    }
