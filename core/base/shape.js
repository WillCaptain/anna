 ...
 *
 * 职责：
 *   1. 持有所有可序列化的图形属性（坐标、尺寸、颜色、文字、行为标志等）
 *   2. 管理 Connector 列表（resize 点、link 点）
 *   3. 持有 Drawer 引用（渲染与 Shape 数据分离）
 *   4. 提供类型查询方法（isType / isTypeof）
 *   5. 提供 select/unSelect/focus/unfocus 等交互语义方法
 *   6. 提供序列化/反序列化（serialize/deSerialize）
 *   7. 支持 JS 代码钩子（loadCode、clickCode 等）在特定时机执行
 *
 * 坐标系：x/y 使用绝对坐标（相对画布左上角），与是否在 Container 内无关。
 *
 * 不应直接实例化此基类，应使用具体子类（rectangle、ellipse 等）。
 *
 * @param {string|null} id     图形唯一 id，不传时自动生成 UUID
 * @param {number} x           绝对横坐标
 * @param {number} y           绝对纵坐标
 * @param {number} width       宽度
 * @param {number} height      高度
 * @param {Shape|Page|Graph}  parent  父容器（提供 graph/page/id 引用）
 * @param {function} drawer    绘制器工厂函数，签名：(shape, div) => DrawerObject
 */
const shape = (id, x, y, width, height, parent, drawer) => {
  let detections = [];
  let self = new Atom(detections);

  // type 设置会触发 Atom 内部的 typeChain 维护和 serializedFields 初始化
  self.type = 'shape';

  // id 不传时通过 graph.uuid() 生成，确保全局唯一
  self.id = id ? id : parent.graph.uuid();

  if (parent !== undefined && parent !== null) {
    if (parent.newPageMode) {
      // 当 parent 携带 newPageMode 标志时，说明当前正在创建一个 Page 对象本身
      // Page 既是 Shape 也是最顶层容器，self.page 指向自身
      self.page = self;
      self.mode = parent.newPageMode;
    } else {
      // 普通 Shape：从 parent 继承 graph/page 引用，container 记录所属容器 id
  ...
  // typeChain 在此处由 Atom 的 Proxy 拦截 type 赋值时自动维护
  // 此处的手动赋值用于初始化 'shape' 根节点
  self.typeChain = {
    parent: null, type: self.type,
  };

  /**
   * 精确类型匹配：仅匹配 typeChain 的最末端（当前类型）。
   * 例：rectangle 实例调用 isType('shape') 返回 false。
   */
  self.isType = type => {
  ...
  /**
   * 类型继承链匹配：沿 typeChain 向上遍历，匹配任意祖先类型。
   * 例：rectangle 实例调用 isTypeof('shape') 返回 true。
   */
  self.isTypeof = type => {
  ...
  // ---- 静态属性（不参与序列化，由代码固定设置） ----
  // namespace：形状所属的命名空间，内置形状为 'anna'，插件形状使用自定义 namespace
  self.namespace = 'anna';
  // serializable：false 时该形状不会被序列化（如临时辅助形状）
  self.serializable = true;
  self.defaultWidth = 100;
  self.defaultHeight = 100;
  // shareAble：true 表示该形状可以被多个 Page 通过 reference 引用（跨页共享）
  self.shareAble = false;
  // needLevitation：批注场景下，该形状是否需要随文字位置自适应浮动
  self.needLevitation = true;

  // 只有普通 Shape 才创建 Drawer；Page 对象本身（newPageMode 存在时）不在此处创建，
  // 因为 Page 的 Drawer 在 page.js 中由多个独立 Drawer 组合创建
  if (!parent.newPageMode) {
    self.drawer = drawer(self, self.page.div);
  }

  /**
   * 沿父容器链向上查找最近的"被共享"的容器（shared === true）。
   * 用于确定跨页引用时该 Shape 属于哪个共享容器。
   * 若 container 为空（已删除），返回 savedSharedParent（删除前保存的引用）。
   */
  self.sharedParent = () => {
    if (self.container === '') {
  ...
  // ---- 动态属性（运行时状态，不序列化） ----
  // 记录选中时的初始坐标，用于拖动过程中计算相对偏移量
  self.selectedX = 0;
  self.selectedY = 0;
  // isFocused：当前是否处于选中（高亮）状态
  self.isFocused = false;
  // linking：当前是否正在被某条 Line 连接（鼠标悬停到 Connector 上时设为 true）
  self.linking = false;
  self.linkingConnector = null;
  // cursor：鼠标悬停到该形状上时显示的光标样式
  self.cursor = CURSORS.MOVE;
  // allowTraced：false 时该形状的操作不记录到 Undo/Redo 历史
  self.allowTraced = true;
  // isAutoSize：true 时 Container 布局时会根据内容自动调整尺寸
  self.isAutoSize = true;
  // allowClickRun：true 时在 display/presentation 模式下单击会执行 clickCode
  self.allowClickRun = false;
  // allowMultiLineEdit：true 时支持多行文字输入
  self.allowMultiLineEdit = false;
  // mousedownConnector：记录当前按下鼠标时命中的 Connector，用于拖拽判断
  self.mousedownConnector = null;

  /**
   * 当前形状是否处于文字编辑状态（光标在文字输入框内）。
   */
  self.isEditing = () => {
    if (!self.drawer.text) {
      return false;
