...
import {addCommand, eraserComamnd, positionCommand, resizeCommand, updateFreeLineCommand} from '../history/commands.js';
...
const mouseEvents = () => {
    ...
    const calculatePosition = (e) => {
        ...
        // 首次事件用当前位置初始化，避免 deltaX/Y 出现巨大值（等于完整画布坐标）
        const zoom = page.interactDrawer.zoom;
        const rect = page.div.getBoundingClientRect();

        if (!mouseActionsRecords[e.type]) {
            mouseActionsRecords[e.type] = {
        ...
        // mouseX/Y 是鼠标相对于 page.div 左上角的视口像素偏移量；
        // calculateX/Y 再除以 zoom/scaleX 转换为画布内部坐标。
    ...
    const calculateY = (mouseY, zoom, scrollPosition) => {
        const rect = page.div.getBoundingClientRect();
        const oh = page.div.offsetHeight;
    ...
    let events = {
        init: (p) => {
            ...
            element.addEventListener('wheel', addMouseListener(mouseWheelListener), {passive: false});
...
const bindMouseActions = page => {
    ...
    function generateCommand(position) {
        ...
        position.context = {command: undefined, shapes: []};
    ...
    events.onMouseMove(position => {
        // 如果存在activeElement，则将事件处理交由activeElement直接处理，anna的事件机制不进行处理.
    ...
    events.onMouseOut(_position => {
        // mouseout 事件存在误触发问题，暂时禁用（参见 mouseLeave 处理）
    });
    ...
    /**
     * 添加mouseLeave事件.
     * 用于规避拖动图形超出画布时，anna无法响应鼠标mouseUp事件，之后无法选中该图形的问题.
     *
     * * 注意 *
