...
import {addCommand} from '../history/commands.js';
 ...
 * Anna数据拷贝处理器.
 */
export class AnnaCopyHandler {
    static ANNA_COPY_MATCHER = /.*anna\/(?<type>\w+)/;
    ...
    handle(event, items, page) {
        const annaCopy = items.map(item => {
            return {item, match: AnnaCopyHandler.ANNA_COPY_MATCHER.exec(item.type)};
        }).find(m => !!m.match);
        if (!annaCopy) {
            return;
        }
        const focused = page.getFocusedShapes();
        const arg = event.clipboardData.getData(annaCopy.item.type);
        ...
        const handledByShape = annaCopy.match.groups.type !== "shape" && isOneFocused && isPasteOk;
        if (!handledByShape) {
...
export class PlainCopyHandler {
    ...
    _convertShapes(arg) {
        ...
        if (data.type && data.type === "anna" && data.shapes.length > 0) {
            return data.shapes;
        }
        return null;
    }
...
export class ImageCopyHandler {
    ...
    _getImgUrl(imageURI) {
        ...
        return new Promise((resolve, reject) => {
            ...
            httpRequest.open('post', window.location.origin + "/anna-backend/common/upload");
