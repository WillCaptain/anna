...
import {reGenerateId} from "../../common/annaEntryUtil.js";
import {defaultGraph} from "../base/defaultGraph.js";
import {annatoimage} from "../../common/anna2image.js";
import {defaultRepository} from "../repository/defaultRepository.js";
import {annaWriter} from "../base/writer.js";
...
const graphAgent = (graph, enableDebug, repo) => {
    ...
    agent.initGraphWithTemplate = async (data) => {
        ...
        ANNA.convertGraphData(cloneData);
    ...
    agent.present = async (index) => {
        ...
        const presentGraph = await ANNA.presentGraph(graph.id, graph.type, viewer, index, graph.serialize());
    ...
    agent.change = (operation) => {
        ...
        graph.change(() => {
            !writer && (writer = annaWriter(graph));
            operation(writer);
        });
    }
...
const ANNA = (() => {
    ...
    self.displayGraph = async (graphId, graphType, div) => {
        const g = await emptyGraph(graphType, div);
        await g.initialize();
        const data = await repo.getGraph(graphId);
        g.deSerialize(data);
        return g;
    };
