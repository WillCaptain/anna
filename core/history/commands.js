 ...
 * Command — 所有操作指令的基类。
 *
 * Command 模式：每次用户操作（移动/删除/新增等）创建一个 Command 对象，
 * 该对象同时记录操作前后的数据快照，支持 redo（重做）和 undo（撤销）。
 *
 * shapes 参数格式示例：
 *   [{shape: shapeObj, x: {value: 100, preValue: 80}, y: {value: 50, preValue: 30}}]
 * 在 serializeShapes 后，shape 引用被替换为 shape.id（字符串），
 * redo/undo 执行时再通过 host.sm.getShapeById(id) 重新获取运行时对象。
 *
 * @param {Page|Graph} host   Command 的宿主，序列化后存入 self.host
 * @param {Array}      shapes 操作涉及的形状数据数组（含变化前后的值）
 * @param {string}     type   Command 类型标识（如 'position'、'add' 等）
 * @param {function}   init   初始化函数，在 addCommand 入历史栈之前执行
 */
const command = (host, shapes, type, init) => {
  // shapes 序列化后存储（shape 对象引用 → shape.id 字符串），避免内存泄漏
  const self = {shapes: serializeShapes(shapes)};
  self.type = type;

  /**
   * 重做：执行操作（将数据从 preValue 恢复到 value）
   * @param {Page|Graph} host 当前激活的宿主
   */
  self.redo = (host) => {
  ...
  /**
   * 撤销：逆向执行操作（将数据从 value 恢复到 preValue）
   * @param {Page|Graph} host 当前激活的宿主
   */
  self.undo = (host) => {
  ...
  /**
   * execute：Command 创建时的首次执行（适用于需要延迟执行的场景，如 transaction）。
   * 注意：并非所有 Command 都实现此方法，大多数 Command 在创建时已直接完成操作。
   */
  self.execute = () => {
  };

  init && init();
  // host 序列化存储（快照），undo/redo 时用来还原页面状态
...
/**
 * transactionCommand — 事务 Command，将多个 Command 组合为一个原子操作。
 *
 * 创建时会将 commands 参数中的所有子 Command 从历史栈中移除（它们已各自入栈），
 * 只保留本事务 Command，确保 Ctrl+Z 时一次性撤销整组操作。
 *
 * @param {Page|Graph} page        宿主
 * @param {Command[]}  commands    子命令数组
 * @param {boolean}    strictOrder true 时 undo 按严格逆序执行（有依赖关系时用）
 */
const transactionCommand = (page, commands, strictOrder) => {
  const h = page.graph ? page.graph : page;
  // init 函数：从历史栈中移除各子 Command（因为它们在创建时已各自入栈）
...
/**
 * 将 shapes 数组序列化：把 shape 对象引用替换为 shape.id 字符串。
 * 原因：Command 需要跨页存储，直接持有运行时对象会阻止 GC（内存泄漏）。
 * undo/redo 时通过 host.sm.getShapeById(id) 重新获取运行时对象。
 */
...
/**
 * positionCommand — 形状位置移动 Command。
 *
 * shapes 数据格式：[{shape, x:{value,preValue}, y:{value,preValue}, container:{value,preValue}}]
 * undo：将坐标恢复到 preValue，redo：恢复到 value。
 * 同时处理 container 变化（拖入/拖出 Container 时容器也会变化）。
 */
...
/**
 * 编辑器指令。所有的操作统一由anna管理，因此这里需要一个命令来触发编辑器的历史管理功能。
 *
 * @param page 页面对象。
 * @param editor 编辑器对象。
 * @param data 数据。
