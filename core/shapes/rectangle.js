...
import * as regions from '../interaction/hitRegion.js';
...
const rectangle = (id, x, y, width, height, parent, drawer = rectangleDrawer) => {
  ...
  self.initConnectors = () => {
    ...
    let release = position => {
      if (self.isNew) {
        // 若用户只点击未拖拽（尺寸过小），恢复到 beginDrag 前保存的默认尺寸
        if (self.width < MIN) {
          self.width = self._defaultWidth || MIN;
        }
        if (self.height < MIN) {
          self.height = self._defaultHeight || MIN;
        }
        delete self.isNew;
        delete self._defaultWidth;
        delete self._defaultHeight;
